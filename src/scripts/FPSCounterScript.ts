...
export class FPSCounterScript extends Script {
    ...
    private initializeFPSMeter() {
        const attemptInit = () => {
            if (typeof window.FPSMeter !== 'undefined') {
                ...
                console.log("[FPSCounterScript] 📊 FPSMeter initialized");
            } else {
    ...
    destroy() {
        if (this.fps && this.fps.destroy) {
            this.fps.destroy();
            console.log("[FPSCounterScript] 🧹 FPSMeter destroyed");
        }
    }
}

export default FPSCounterScript;